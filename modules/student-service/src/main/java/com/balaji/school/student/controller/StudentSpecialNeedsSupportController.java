...
import com.balaji.school.student.service.StudentSpecialNeedsSupportService;
...
/**
 * Enterprise REST API Controller for StudentSpecialNeedsSupport.
 */
public class StudentSpecialNeedsSupportController {

    private final StudentSpecialNeedsSupportService service;

    public StudentSpecialNeedsSupportController() {
        this.service = new StudentSpecialNeedsSupportService();
    }

    public StudentSpecialNeedsSupportController(StudentSpecialNeedsSupportService service) {
        this.service = service != null ? service : new StudentSpecialNeedsSupportService();
    }

    public Map<String, Object> fetchAll() {
        List<StudentSpecialNeedsSupportEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<StudentSpecialNeedsSupportEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(StudentSpecialNeedsSupportRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            StudentSpecialNeedsSupportEntity created = service.registerEntry(request);
