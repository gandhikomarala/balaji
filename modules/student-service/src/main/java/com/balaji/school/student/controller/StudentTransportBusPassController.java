...
import com.balaji.school.student.service.StudentTransportBusPassService;
...
/**
 * Enterprise REST API Controller for StudentTransportBusPass.
 */
public class StudentTransportBusPassController {

    private final StudentTransportBusPassService service;

    public StudentTransportBusPassController() {
        this.service = new StudentTransportBusPassService();
    }

    public StudentTransportBusPassController(StudentTransportBusPassService service) {
        this.service = service != null ? service : new StudentTransportBusPassService();
    }

    public Map<String, Object> fetchAll() {
        List<StudentTransportBusPassEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<StudentTransportBusPassEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(StudentTransportBusPassRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            StudentTransportBusPassEntity created = service.registerEntry(request);
