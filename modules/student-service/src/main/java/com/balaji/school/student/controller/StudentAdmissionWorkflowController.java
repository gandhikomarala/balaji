...
import com.balaji.school.student.service.StudentAdmissionWorkflowService;
...
/**
 * Enterprise REST API Controller for StudentAdmissionWorkflow.
 */
public class StudentAdmissionWorkflowController {

    private final StudentAdmissionWorkflowService service;

    public StudentAdmissionWorkflowController() {
        this.service = new StudentAdmissionWorkflowService();
    }

    public StudentAdmissionWorkflowController(StudentAdmissionWorkflowService service) {
        this.service = service != null ? service : new StudentAdmissionWorkflowService();
    }

    public Map<String, Object> fetchAll() {
        List<StudentAdmissionWorkflowEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<StudentAdmissionWorkflowEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(StudentAdmissionWorkflowRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            StudentAdmissionWorkflowEntity created = service.registerEntry(request);
