...
import com.balaji.school.student.service.StudentHostelAllocationService;
...
/**
 * Enterprise REST API Controller for StudentHostelAllocation.
 */
public class StudentHostelAllocationController {

    private final StudentHostelAllocationService service;

    public StudentHostelAllocationController() {
        this.service = new StudentHostelAllocationService();
    }

    public StudentHostelAllocationController(StudentHostelAllocationService service) {
        this.service = service != null ? service : new StudentHostelAllocationService();
    }

    public Map<String, Object> fetchAll() {
        List<StudentHostelAllocationEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<StudentHostelAllocationEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(StudentHostelAllocationRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            StudentHostelAllocationEntity created = service.registerEntry(request);
