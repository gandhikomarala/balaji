...
import com.balaji.school.student.service.StudentMiddayMealTokenService;
...
/**
 * Enterprise REST API Controller for StudentMiddayMealToken.
 */
public class StudentMiddayMealTokenController {

    private final StudentMiddayMealTokenService service;

    public StudentMiddayMealTokenController() {
        this.service = new StudentMiddayMealTokenService();
    }

    public StudentMiddayMealTokenController(StudentMiddayMealTokenService service) {
        this.service = service != null ? service : new StudentMiddayMealTokenService();
    }

    public Map<String, Object> fetchAll() {
        List<StudentMiddayMealTokenEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<StudentMiddayMealTokenEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(StudentMiddayMealTokenRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            StudentMiddayMealTokenEntity created = service.registerEntry(request);
