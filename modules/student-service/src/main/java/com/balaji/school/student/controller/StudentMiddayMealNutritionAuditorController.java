...
import com.balaji.school.student.service.StudentMiddayMealNutritionAuditorService;
...
/**
 * Enterprise REST API Controller for StudentMiddayMealNutritionAuditor.
 */
public class StudentMiddayMealNutritionAuditorController {

    private final StudentMiddayMealNutritionAuditorService service;

    public StudentMiddayMealNutritionAuditorController() {
        this.service = new StudentMiddayMealNutritionAuditorService();
    }

    public StudentMiddayMealNutritionAuditorController(StudentMiddayMealNutritionAuditorService service) {
        this.service = service != null ? service : new StudentMiddayMealNutritionAuditorService();
    }

    public Map<String, Object> listAll() {
        List<StudentMiddayMealNutritionAuditorEntity> records = service.retrieveAllRecords();
    ...
    public Map<String, Object> getById(String id) {
        Optional<StudentMiddayMealNutritionAuditorEntity> entity = service.retrieveById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(StudentMiddayMealNutritionAuditorRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            StudentMiddayMealNutritionAuditorEntity created = service.executeWorkflow(request);
