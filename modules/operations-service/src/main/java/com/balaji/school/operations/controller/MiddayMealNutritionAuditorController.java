...
import com.balaji.school.operations.service.MiddayMealNutritionAuditorService;
...
/**
 * Enterprise REST API Controller for MiddayMealNutritionAuditor.
 */
public class MiddayMealNutritionAuditorController {

    private final MiddayMealNutritionAuditorService service;

    public MiddayMealNutritionAuditorController() {
        this.service = new MiddayMealNutritionAuditorService();
    }

    public MiddayMealNutritionAuditorController(MiddayMealNutritionAuditorService service) {
        this.service = service != null ? service : new MiddayMealNutritionAuditorService();
    }

    public Map<String, Object> fetchAll() {
        List<MiddayMealNutritionAuditorEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<MiddayMealNutritionAuditorEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(MiddayMealNutritionAuditorRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            MiddayMealNutritionAuditorEntity created = service.registerEntry(request);
