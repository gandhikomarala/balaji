...
import com.balaji.school.operations.service.HostelMessSanitationProtocolService;
...
/**
 * Enterprise REST API Controller for HostelMessSanitationProtocol.
 */
public class HostelMessSanitationProtocolController {

    private final HostelMessSanitationProtocolService service;

    public HostelMessSanitationProtocolController() {
        this.service = new HostelMessSanitationProtocolService();
    }

    public HostelMessSanitationProtocolController(HostelMessSanitationProtocolService service) {
        this.service = service != null ? service : new HostelMessSanitationProtocolService();
    }

    public Map<String, Object> fetchAll() {
        List<HostelMessSanitationProtocolEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<HostelMessSanitationProtocolEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(HostelMessSanitationProtocolRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            HostelMessSanitationProtocolEntity created = service.registerEntry(request);
