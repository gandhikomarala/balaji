...
import com.balaji.school.operations.service.CampusSecurityVisitorGateService;
...
/**
 * Enterprise REST API Controller for CampusSecurityVisitorGate.
 */
public class CampusSecurityVisitorGateController {

    private final CampusSecurityVisitorGateService service;

    public CampusSecurityVisitorGateController() {
        this.service = new CampusSecurityVisitorGateService();
    }

    public CampusSecurityVisitorGateController(CampusSecurityVisitorGateService service) {
        this.service = service != null ? service : new CampusSecurityVisitorGateService();
    }

    public Map<String, Object> fetchAll() {
        List<CampusSecurityVisitorGateEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<CampusSecurityVisitorGateEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(CampusSecurityVisitorGateRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            CampusSecurityVisitorGateEntity created = service.registerEntry(request);
