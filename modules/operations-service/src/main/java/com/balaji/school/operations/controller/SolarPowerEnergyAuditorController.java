...
import com.balaji.school.operations.service.SolarPowerEnergyAuditorService;
...
/**
 * Enterprise REST API Controller for SolarPowerEnergyAuditor.
 */
public class SolarPowerEnergyAuditorController {

    private final SolarPowerEnergyAuditorService service;

    public SolarPowerEnergyAuditorController() {
        this.service = new SolarPowerEnergyAuditorService();
    }

    public SolarPowerEnergyAuditorController(SolarPowerEnergyAuditorService service) {
        this.service = service != null ? service : new SolarPowerEnergyAuditorService();
    }

    public Map<String, Object> fetchAll() {
        List<SolarPowerEnergyAuditorEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<SolarPowerEnergyAuditorEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(SolarPowerEnergyAuditorRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            SolarPowerEnergyAuditorEntity created = service.registerEntry(request);
