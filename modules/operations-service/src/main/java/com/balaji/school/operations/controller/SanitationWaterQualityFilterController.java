...
import com.balaji.school.operations.service.SanitationWaterQualityFilterService;
...
/**
 * Enterprise REST API Controller for SanitationWaterQualityFilter.
 */
public class SanitationWaterQualityFilterController {

    private final SanitationWaterQualityFilterService service;

    public SanitationWaterQualityFilterController() {
        this.service = new SanitationWaterQualityFilterService();
    }

    public SanitationWaterQualityFilterController(SanitationWaterQualityFilterService service) {
        this.service = service != null ? service : new SanitationWaterQualityFilterService();
    }

    public Map<String, Object> fetchAll() {
        List<SanitationWaterQualityFilterEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<SanitationWaterQualityFilterEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(SanitationWaterQualityFilterRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            SanitationWaterQualityFilterEntity created = service.registerEntry(request);
