...
import com.balaji.school.operations.service.ClassroomSmartTvAssetDeskService;
...
/**
 * Enterprise REST API Controller for ClassroomSmartTvAssetDesk.
 */
public class ClassroomSmartTvAssetDeskController {

    private final ClassroomSmartTvAssetDeskService service;

    public ClassroomSmartTvAssetDeskController() {
        this.service = new ClassroomSmartTvAssetDeskService();
    }

    public ClassroomSmartTvAssetDeskController(ClassroomSmartTvAssetDeskService service) {
        this.service = service != null ? service : new ClassroomSmartTvAssetDeskService();
    }

    public Map<String, Object> fetchAll() {
        List<ClassroomSmartTvAssetDeskEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<ClassroomSmartTvAssetDeskEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(ClassroomSmartTvAssetDeskRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            ClassroomSmartTvAssetDeskEntity created = service.registerEntry(request);
