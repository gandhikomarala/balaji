...
import com.balaji.school.security.service.ParentPortalAccessGatekeeperService;
...
/**
 * REST API Endpoint Controller for ParentPortalAccessGatekeeper.
 */
public class ParentPortalAccessGatekeeperController {

    private final ParentPortalAccessGatekeeperService service;

    public ParentPortalAccessGatekeeperController() {
        this.service = new ParentPortalAccessGatekeeperService();
    }

    public ParentPortalAccessGatekeeperController(ParentPortalAccessGatekeeperService service) {
        this.service = service != null ? service : new ParentPortalAccessGatekeeperService();
    }

    public Map<String, Object> listAll() {
        List<ParentPortalAccessGatekeeperEntity> records = service.getAll();
    ...
    public Map<String, Object> findById(String id) {
        Optional<ParentPortalAccessGatekeeperEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> createEntry(ParentPortalAccessGatekeeperRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            ParentPortalAccessGatekeeperEntity created = service.executeWorkflow(request);
