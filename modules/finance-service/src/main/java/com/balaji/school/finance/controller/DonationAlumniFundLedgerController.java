...
import com.balaji.school.finance.service.DonationAlumniFundLedgerService;
...
/**
 * Enterprise REST API Controller for DonationAlumniFundLedger.
 */
public class DonationAlumniFundLedgerController {

    private final DonationAlumniFundLedgerService service;

    public DonationAlumniFundLedgerController() {
        this.service = new DonationAlumniFundLedgerService();
    }

    public DonationAlumniFundLedgerController(DonationAlumniFundLedgerService service) {
        this.service = service != null ? service : new DonationAlumniFundLedgerService();
    }

    public Map<String, Object> fetchAll() {
        List<DonationAlumniFundLedgerEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<DonationAlumniFundLedgerEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(DonationAlumniFundLedgerRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            DonationAlumniFundLedgerEntity created = service.registerEntry(request);
