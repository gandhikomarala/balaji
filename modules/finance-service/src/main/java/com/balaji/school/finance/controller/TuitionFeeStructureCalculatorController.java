...
import com.balaji.school.finance.service.TuitionFeeStructureCalculatorService;
...
/**
 * Enterprise REST API Controller for TuitionFeeStructureCalculator.
 */
public class TuitionFeeStructureCalculatorController {

    private final TuitionFeeStructureCalculatorService service;

    public TuitionFeeStructureCalculatorController() {
        this.service = new TuitionFeeStructureCalculatorService();
    }

    public TuitionFeeStructureCalculatorController(TuitionFeeStructureCalculatorService service) {
        this.service = service != null ? service : new TuitionFeeStructureCalculatorService();
    }

    public Map<String, Object> fetchAll() {
        List<TuitionFeeStructureCalculatorEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<TuitionFeeStructureCalculatorEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(TuitionFeeStructureCalculatorRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            TuitionFeeStructureCalculatorEntity created = service.registerEntry(request);
