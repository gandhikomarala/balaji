...
import com.balaji.school.academic.service.LibraryBookCirculationDeskService;
...
/**
 * Enterprise REST API Controller for LibraryBookCirculationDesk.
 */
public class LibraryBookCirculationDeskController {

    private final LibraryBookCirculationDeskService service;

    public LibraryBookCirculationDeskController() {
        this.service = new LibraryBookCirculationDeskService();
    }

    public LibraryBookCirculationDeskController(LibraryBookCirculationDeskService service) {
        this.service = service != null ? service : new LibraryBookCirculationDeskService();
    }

    public Map<String, Object> fetchAll() {
        List<LibraryBookCirculationDeskEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<LibraryBookCirculationDeskEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(LibraryBookCirculationDeskRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            LibraryBookCirculationDeskEntity created = service.registerEntry(request);
