...
import com.balaji.school.academic.service.DigitalHomeworkAssignmentService;
...
/**
 * Enterprise REST API Controller for DigitalHomeworkAssignment.
 */
public class DigitalHomeworkAssignmentController {

    private final DigitalHomeworkAssignmentService service;

    public DigitalHomeworkAssignmentController() {
        this.service = new DigitalHomeworkAssignmentService();
    }

    public DigitalHomeworkAssignmentController(DigitalHomeworkAssignmentService service) {
        this.service = service != null ? service : new DigitalHomeworkAssignmentService();
    }

    public Map<String, Object> fetchAll() {
        List<DigitalHomeworkAssignmentEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<DigitalHomeworkAssignmentEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(DigitalHomeworkAssignmentRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            DigitalHomeworkAssignmentEntity created = service.registerEntry(request);
