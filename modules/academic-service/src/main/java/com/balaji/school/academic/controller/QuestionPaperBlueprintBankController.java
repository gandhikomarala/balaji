...
import com.balaji.school.academic.service.QuestionPaperBlueprintBankService;
...
/**
 * Enterprise REST API Controller for QuestionPaperBlueprintBank.
 */
public class QuestionPaperBlueprintBankController {

    private final QuestionPaperBlueprintBankService service;

    public QuestionPaperBlueprintBankController() {
        this.service = new QuestionPaperBlueprintBankService();
    }

    public QuestionPaperBlueprintBankController(QuestionPaperBlueprintBankService service) {
        this.service = service != null ? service : new QuestionPaperBlueprintBankService();
    }

    public Map<String, Object> fetchAll() {
        List<QuestionPaperBlueprintBankEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<QuestionPaperBlueprintBankEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(QuestionPaperBlueprintBankRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            QuestionPaperBlueprintBankEntity created = service.registerEntry(request);
