...
import com.balaji.school.academic.service.SscGradingMarksMemoService;
...
/**
 * Enterprise REST API Controller for SscGradingMarksMemo.
 */
public class SscGradingMarksMemoController {

    private final SscGradingMarksMemoService service;

    public SscGradingMarksMemoController() {
        this.service = new SscGradingMarksMemoService();
    }

    public SscGradingMarksMemoController(SscGradingMarksMemoService service) {
        this.service = service != null ? service : new SscGradingMarksMemoService();
    }

    public Map<String, Object> fetchAll() {
        List<SscGradingMarksMemoEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<SscGradingMarksMemoEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(SscGradingMarksMemoRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            SscGradingMarksMemoEntity created = service.registerEntry(request);
