...
import com.balaji.school.academic.service.MasterTimetableConstraintSolverService;
...
/**
 * Enterprise REST API Controller for MasterTimetableConstraintSolver.
 */
public class MasterTimetableConstraintSolverController {

    private final MasterTimetableConstraintSolverService service;

    public MasterTimetableConstraintSolverController() {
        this.service = new MasterTimetableConstraintSolverService();
    }

    public MasterTimetableConstraintSolverController(MasterTimetableConstraintSolverService service) {
        this.service = service != null ? service : new MasterTimetableConstraintSolverService();
    }

    public Map<String, Object> fetchAll() {
        List<MasterTimetableConstraintSolverEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<MasterTimetableConstraintSolverEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(MasterTimetableConstraintSolverRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            MasterTimetableConstraintSolverEntity created = service.registerEntry(request);
