...
import com.balaji.school.communication.service.ParentGrievanceEscalationServiceService;
...
/**
 * Enterprise REST API Controller for ParentGrievanceEscalationService.
 */
public class ParentGrievanceEscalationServiceController {

    private final ParentGrievanceEscalationServiceService service;

    public ParentGrievanceEscalationServiceController() {
        this.service = new ParentGrievanceEscalationServiceService();
    }

    public ParentGrievanceEscalationServiceController(ParentGrievanceEscalationServiceService service) {
        this.service = service != null ? service : new ParentGrievanceEscalationServiceService();
    }

    public Map<String, Object> listAll() {
        List<ParentGrievanceEscalationServiceEntity> records = service.retrieveAllRecords();
    ...
    public Map<String, Object> getById(String id) {
        Optional<ParentGrievanceEscalationServiceEntity> entity = service.retrieveById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(ParentGrievanceEscalationServiceRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            ParentGrievanceEscalationServiceEntity created = service.executeWorkflow(request);
