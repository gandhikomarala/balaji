...
import com.balaji.school.communication.service.BalajiAlumniAssociationNetworkService;
...
/**
 * Enterprise REST API Controller for BalajiAlumniAssociationNetwork.
 */
public class BalajiAlumniAssociationNetworkController {

    private final BalajiAlumniAssociationNetworkService service;

    public BalajiAlumniAssociationNetworkController() {
        this.service = new BalajiAlumniAssociationNetworkService();
    }

    public BalajiAlumniAssociationNetworkController(BalajiAlumniAssociationNetworkService service) {
        this.service = service != null ? service : new BalajiAlumniAssociationNetworkService();
    }

    public Map<String, Object> fetchAll() {
        List<BalajiAlumniAssociationNetworkEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<BalajiAlumniAssociationNetworkEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(BalajiAlumniAssociationNetworkRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            BalajiAlumniAssociationNetworkEntity created = service.registerEntry(request);
