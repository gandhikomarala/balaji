...
import com.balaji.school.communication.service.ParentTeacherMeetingSchedulerService;
...
/**
 * Enterprise REST API Controller for ParentTeacherMeetingScheduler.
 */
public class ParentTeacherMeetingSchedulerController {

    private final ParentTeacherMeetingSchedulerService service;

    public ParentTeacherMeetingSchedulerController() {
        this.service = new ParentTeacherMeetingSchedulerService();
    }

    public ParentTeacherMeetingSchedulerController(ParentTeacherMeetingSchedulerService service) {
        this.service = service != null ? service : new ParentTeacherMeetingSchedulerService();
    }

    public Map<String, Object> fetchAll() {
        List<ParentTeacherMeetingSchedulerEntity> records = service.getAll();
    ...
    public Map<String, Object> fetchById(String id) {
        Optional<ParentTeacherMeetingSchedulerEntity> entity = service.getById(id);
        Map<String, Object> response = new HashMap<>();
        if (entity.isPresent()) {
            response.put("statusCode", 200);
    ...
    public Map<String, Object> submitRecord(ParentTeacherMeetingSchedulerRequestDto request) {
        Map<String, Object> response = new HashMap<>();
        try {
            ParentTeacherMeetingSchedulerEntity created = service.registerEntry(request);
